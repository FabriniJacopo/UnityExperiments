...
Shader "Custom/VHSeffect"
{
	Properties
	{
		...
		_OurAmount ("Our amount", float) = 0.0
		...
		_ColorAmount ("Color Amount", float) = 0.0
	...
	SubShader
	{
		Pass
		{
			...
			{
				i.uv = float2(frac(i.uv.x + cos((i.uv.y + _CosTime.y) * _OurAmount) / _OffsetDistortion), frac(i.uv.y + _OffsetPosY));
				
				fixed4 col = tex2D(_MainTex, i.uv);
				_OffsetColor*=abs(sin(_Time.y))*_ColorAmount;
